...
### ⚠️ 坑：`build-blog.js` 从 `wallpaper/` 复制到 `dist/`，所以必须改源码

**问题现象**：
修改 `generate-wallpapers.cjs` 的 `CAT_NAME_ZH` 映射表后，如果只手动改 `dist/wallpaper/*/index.zh.html`，Vercel 构建时 `build-blog.js` 会从根目录 `wallpaper/` 重新复制，覆盖掉修改。

**正确流程**：
1. 修改 `scripts/generate-wallpapers.cjs`（添加/修改 `CAT_NAME_ZH` 映射）
2. 跑 `node scripts/generate-wallpapers.cjs --id=XXX`（重新生成目标页面到 `wallpaper/` 源码目录）
3. **或者**直接批量替换 `wallpaper/*/index.zh.html` 中的文本（如果只是模板文本变更）
4. 跑 `node build-blog.js`（从 `wallpaper/` 复制到 `dist/`）
5. 清理 `dist/wallpaper/*/index.zh/` 重复目录
6. `git add wallpaper/` + `git add -f dist/` + `git commit` + 手动 push

**原因**：
- `build-blog.js` 第 2770 行：`copyDir(wallpaperSrcDir, wallpaperDestDir)` — 从 `wallpaper/` 复制到 `dist/wallpaper/`
- `dist/` 在 `.gitignore` 中，需要用 `git add -f` 强制添加

**教训**：
- **永远不要只改 `dist/`，必须同时改 `wallpaper/` 源码**
- 每次 `build-blog.js` 后必须检查 `index.zh/` 目录并删除（已加入流程）

---

## 壁纸上线流程（重要 · 已锁定 · 不可擅自修改）

> 🔒 **此流程已锁死，没有用户明确命令（"跑图"），不得执行、不得修改、不得优化。**
> 用户说"跑图"以外的任何话（如"准备""上新""看看"等）均**不触发**此流程。

**触发词**：用户说 **"跑图"** → 自动执行以下流程并 **自动 push**（此为例外，其余功能仍须手动 push）
...
- ✅ `build-blog.js` Step 5 和 Step 9 已改为增量，只处理新增壁纸
- ✅ 如果用户脚本已自动 push，则只跑 SEO 再追加 push
- ❌ **没有用户明确命令，不得擅自修改、优化、重构此流程的任何环节**
