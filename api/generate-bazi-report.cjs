...
function buildSystemPrompt(section) {
  ...
  const prompts = {
    ...
    dayun: coreRules + ' ' + formatRules + ' ' + antiFabrication + noEmoji +
      '【绝对禁止越界】本章只讲大运干支分析和吉凶判定，禁止展开事业/财运/感情/健康的具体建议，那些是后面章节的事。\n' +
      ...
      '【天干分析】（必寫，不可省略）\n' +
      '  天干十神是什麼，對命主有何影響。\n' +
      '  天干十神是什么，对整体运势的影响（一句话即可，不展开具体领域）。\n\n' +
      '【地支分析】\n' +
      '  1. 地支藏干有哪些，对整体运势的影响（一句话即可）。\n' +
      '  2. 地支与原局是否有合/冲/刑/害，具体说明。\n' +
      '  3. 天干+地支综合判断：吉凶理由（2-3句话）。\n\n' +
      '【本运重点】用2-3句话总结这步大运最需要注意的1-2件事。\n\n' +
      '控制在1000字以内。',
...
重要提示：此命為**身弱**格局（己土生申月失令，辰為濕土不助土反助水，壬水透干耗身）。身弱行比劫運（戊辰）為吉——比劫幫身助日主擔財。雖然辰為濕土略有晦火之弊，但天干戊土幫身是主導力量，**整體為吉**，不可判為「先吉後凶」。身弱行印運（己巳、丙寅、丁卯）亦為吉。請嚴格按照此判斷分析所有大運。
