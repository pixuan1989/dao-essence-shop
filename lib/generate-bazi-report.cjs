...
function runPaipan(yy, mm, dd, hh, gender) {
  ...
  const hIdx = Math.floor((safeHh + 1) / 2) % 12;
...
async function generateBaziReport(orderData) {
  ...
  const hh = parseInt(orderData.birthHour) || 0; // 防御 NaN/空串，默认子时
  ...
  const baziData = {
    ...
    birthHour: `${paipanResult.hourName || '子'}時 (${hh}:00-${hh+1}:00)`,
...
module.exports = { generateBaziReport, runPaipan, fillTemplate };
