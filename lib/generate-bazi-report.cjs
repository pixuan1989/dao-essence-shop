...
const { recordUsage } = require('./ai-usage.cjs');
const vm = require('vm');
  ...
  for (let attempt = 0; attempt <= retries; attempt++) {
    try {
      ...
      if (data.choices?.[0]?.message?.content) {
        // 记账：每次成功调用记一行（用于和阿里云账单对账）
        recordUsage({
          script: 'lib/generate-bazi-report.cjs',
          model: DASHSCOPE_MODEL,
          purpose: 'bazi-report',
          usage: data.usage,
          ref: options.ref || '',
...
function buildSystemPrompt(section, sectionKnowledge = '') {
  ...
  const prompts = {
    ...
    overview: coreRules + ' ' + formatRules + ' ' + antiFabrication + noEmoji + 
      ...
      '在總評之後，補充以下真實解讀維度（均須基於上方排盤數據，禁止發明）：\n' +
      '· 四柱分柱象義：年柱（祖上根基/早年環境）、月柱（父母家庭/成長土壤）、日柱（自我與配偶）、時柱（子女/晚年/外在表現）各自的象。\n' +
      '· 納音五行：年柱納音的含義與對命主的隱性影響。\n' +
      '· 日主與月令關係：得令與否對全局的定調。\n' +
      '· 一句話總結賓主體用與做功結論，呼應後文。\n' +
      '控制在1100字以內。',
    ...
    personality: coreRules + ' ' + formatRules + ' ' + antiFabrication + noEmoji + 
      ...
      '在基本性格之外，補充以下真實維度（基於日主與十神，禁止空話）：\n' +
      '· 三種場景下的性格呈現：職場場合、親密關係、獨處與壓力情境各如何表現。\n' +
      '· 性格的兩面性：同一特質在何時是優勢、何時淪為陷阱。\n' +
      '· 性格與大運的互動：哪步大運會激發性格的光面、哪步激發暗面。\n' +
      '· 一個具體的「人生課題／修煉方向」。\n' +
      '控制在1300字以內。',
    ...
    geju: coreRules + ' ' + formatRules + ' ' + antiFabrication + noEmoji + 
      ...
      '身強弱如何影響大運吉凶。\n\n' +
      '在以上三部分之外，深入補充（均須結合本命實際，禁止套模板）：\n' +
      '· 格局成格／破格的判定與機理：此格為何成立或不純，破格的力量來自哪柱。\n' +
      '· 用神與喜忌的具體判定：扶抑用神、調候用神、通關用神分別是什麼，用本命舉例。\n' +
      '· 調候：寒暖燥濕是否失衡，調候用神如何取。\n' +
      '· 五行缺旺的具體補救路徑：不是空喊「補水」，而是結合大運指出何時得補、如何補。\n' +
      '· 格局與六親／事業的隱性關聯。\n' +
      '控制在2600字以內。',
    ...
    shishen: coreRules + ' ' + formatRules + ' ' + antiFabrication + noEmoji + 
      '寫「十神逐一解讀」。\n\n' +
      '從此命盤中選出最顯著的4-6個十神來深入分析（不要10個全寫，只寫對命主影響最大的）。\n\n' +
      ...
      '每個十神寫350-450字。語言像分析一個人的不同側面，自然流暢。\n\n' +
      '在逐個十神之後，增寫一段【十神組合】：挑本命中最重要的1-2組十神搭配（如財官相生、傷官見官、印星護身等），\n' +
      '說明這組組合如何相互作用、對命主的綜合影響（正面與隱患都要講）。這是進階解讀，禁止空泛。\n' +
      '控制在2600字以內。',
    // 十神下已合併到十神上，不再分段
    shishenBottom: '',
    ...
    dayun: coreRules + ' ' + formatRules + ' ' + antiFabrication + noEmoji + 
      ...
      '- 行财运 → 凶\n' +
      '- 行食伤运 → 泄身或综合评判\n\n' +
      '只分析当前大运（当前年龄所在）和未来大运，已过去的大运不分析。每个大运用连贯段落写，天干地支融合在一起说：\n\n' +
      '【XX运 XX-XX岁】吉／凶\n' +
      ...
      '感情方面：...\n' +
      '健康方面：...\n\n' +
      '在每步大运的连贯段落中，務必融入以下真實維度（禁止只寫結論不寫機理）：\n' +
      '· 此步大運干支對原局做功的加強或破壞機制（具體哪組干支作用）。\n' +
      '· 引動了哪柱、哪個十神，帶來什麼象。\n' +
      '· 這步大運的「人生主題」與核心課題。\n' +
      '· 關鍵轉折年齡（哪一年是分水嶺）。\n' +
      '· 一步大運的末尾用一句點出「最該把握／最需警惕」的一件事。\n' +
      '控制在3000字以内。',
    // ── 流年（近5年，天机阁流程） ──
    liunian: coreRules + ' ' + formatRules + ' ' + antiFabrication + noEmoji + 
      ...
      '術語第一次出現時加大白話解釋。\n\n' +
      '每年在基本結構之外，補充：\n' +
      '· 此流年與當前所行大運的疊加效應（大運是底色，流年是當年變量）。\n' +
      '· 具體領域的「節點」：事業的關鍵動作時點、財運的進出節奏、感情的關鍵事件窗口。\n' +
      '· 該年「最該做的一件事」與「最忌做的一件事」。\n' +
      '控制在2600字以內。',
    ...
    lifa: coreRules + ' ' + formatRules + ' ' + antiFabrication + noEmoji +
      ...
      '写「事业·财运·感情」。分三个子节：\n\n' +
      '【事業方面】\n' +
      '適合的行業細分（具體領域而非泛泛「技術類」）、走管理路線還是專業路線、適合合夥還是單幹、發展黃金年齡區間、最大優勢與最大坑。給3條具體建議。\n\n' +
      '【財運方面】\n' +
      '財富結構（正財為主還是偏財機會多）、有無財庫及其意義、財運爆發在哪步大運、破財流年預警、投資風格傾向。給3條具體建議。\n\n' +
      '【感情方面】\n' +
      '對感情的態度與優勢、配偶宮的象（配偶大概什麼類型）、相處模式特點、最容易出問題的環節。給3條具體建議。\n\n' +
    ...
    fortune: coreRules + ' ' + formatRules + ' ' + antiFabrication + noEmoji + 
      ...
      '· 日常生活建議（飲食、運動方面的建議）\n\n' +
      '最後1句寄語。語言簡潔實用。\n\n' +
      '在每項之後，補充「為何有效」的一句原理（如為何此方位利你、此顏色呼應何五行），\n' +
      '並給出具體行動清單（如每月哪幾天利某事）、佩戴／布局建議、飲食與運動的具體對應（哪類食材／運動補你所需五行）。\n' +
      '控制在1300字以內。',
    ...
    mangpai: 
      ...
      '控制在1700字以內。禁止Emoji。\n\n' +
      '在標準格式之外，補充：\n' +
      '· 做功鏈的逐字解析（哪個干克哪個干、哪個支穿哪個支，鏈條如何閉合）。\n' +
      '· 功神在哪些大運／流年會被激活發動，發動時命主會經歷什麼。\n' +
      '· 廢神是否有可能轉化為功（條件是什麼）。\n' +
      '· 與前文格局／大運結論呼應，指出盲派視角獨特看到的部分。',

    // ── 結語（使用簡化提示，不含知識庫，減少API負擔） ──
    closing: '你是一位資深命理師，正在寫一份八字報告的結語。語氣平靜有力，像臨別贈言。控制在400字以內。禁止Emoji。' + antiFabrication + noEmoji + 
      '寫「結語」。平靜有力，像臨別贈言，不需要吉凶判斷。可回顧本命最關鍵的一個特質或一處功，給一句前瞻性的寄望。控制在400字以內。'
  };
...
分析從排盤引擎給出的當前大運和未來大運（步數以列表中為準，已過去的不列）。每個大運的格式：
【XX運 XX-XX歲】
...
async function renderPDF(html, outputPath) {
  ...
  try {
    ...
    await page.pdf({
      ...
      margin: { top: '12mm', bottom: '10mm', left: '0mm', right: '0mm' },
      displayHeaderFooter: true,
      headerTemplate: `
        <div style="font-size:8pt; color:#8B0000; width:100%; display:flex; justify-content:space-between; padding:0 20mm; border-bottom:1px solid rgba(212,175,55,0.2); box-sizing:border-box;">
          <span style="font-weight:600;">八字批命書 · 玄珍館</span>
          <span style="color:#D4AF37;"><span class="pageNumber"></span> / <span class="totalPages"></span></span>
        </div>`,
      footerTemplate: `
        <div style="font-size:7pt; color:#aaa; width:100%; text-align:center; border-top:1px solid rgba(212,175,55,0.15); padding-top:2mm;">
          八字命理分析報告 · 私家珍藏 · 非請勿示
        </div>`
    });
...
async function generateBaziReport(orderData) {
  ...
  // 構建大運文字描述，供 AI 使用
  // 只排當前大運 + 未來大運：endAge < 當前年齡 視為已過去，不列入（用戶需求：已過去的不排列）
  const dayunText = paipanResult.dayunList
    .filter(d => d.endAge >= currentAge)
    .map((dy, i) =>
