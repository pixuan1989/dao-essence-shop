...
export async function generateAndEmailReport(orderData) {
    ...
    console.log(`[BaZi] ✅ 邮件已发送至 ${orderData.email} (附件: ${reportKind})`);
...
export async function processNextJob() {
    ...
    if (result.status === 'done') {
        await redisSet(QUEUE_KEY, queue.slice(1)); // 成功后出队
    }
...
async function runJob(id, job) {
    ...
    try {
        ...
        return { id, status: 'pending-retry', error: err.message };
    }
}
...
function buildReportEmailBody(name, baziData, reportKind, lang) {
    const four = baziData
        ...
        : '';
    const htmlNote = reportKind === 'HTML'
        ? (lang === 'zh'
