...
async function getPendingEmails() { return (await redisWithRetry('/GET/pending_auto_reply')).result || []; }
async function removePendingEmail(leadId) { const p = await getPendingEmails(); await redisWithRetry('/SET/pending_auto_reply', 'POST', p.filter(x => x.id !== leadId)); }
async function addSentRecord(leadId) { const s = (await redisWithRetry('/GET/sent_auto_replies')).result || []; s.unshift({ id: leadId, sentAt: new Date().toISOString() }); if (s.length > 500) s.length = 500; await redisWithRetry('/SET/sent_auto_replies', 'POST', s); }
async function isAlreadySent(leadId) { return ((await redisWithRetry('/GET/sent_auto_replies')).result || []).some(s => s.id === leadId); }
async function cleanExpiredRecords() { const p = await getPendingEmails(); const v = p.filter(x => Date.now() - new Date(x.createdAt).getTime() < CONFIG.recordTTL * 1000); if (v.length < p.length) await redisWithRetry('/SET/pending_auto_reply', 'POST', v); }
...
要求：1.引用《滴天髓》或《尚书》解释元素特质。2.结合日主给出具体职业方向（如设计/咨询/医疗等）和感情模式（如包容但易压抑自我）。3.给出具体健康建议（器官 + 季节/习惯）。4.结尾钩子引导完整八字分析。5.中英双语，中文约 180 字，用---分隔。不要列表，不要恐吓，不要 AI 腔。`;
  // Placeholder for Qwen API call - in production this would hit the API
  // For now, returning a structured mock to satisfy the logic flow
...
async function main() {
  ...
  for (const lead of due) {
    try {
      ...
      if (!contentSafetyCheck(interpretation)) { await sendMail(lead.email, 'Your Five Elements Result', 'Fallback content', ''); await addSentRecord(lead.id); await removePendingEmail(lead.id); await incrementDailyCount(); sentCount++; continue; }
